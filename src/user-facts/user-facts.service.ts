...
export class UserFactsService {
  constructor(
    @InjectRepository(UserFacts)
    private userFactsRepository: Repository<UserFacts>,
  ) {}
  ...
  async getAvailableTopics() {
    return Object.keys(Topics).map(name => ({
      id: name,
      imageUrl: `assets/topics/${name.toLowerCase().replace('_', '-')}.jpg`,
    }));
  }
  ...
  async saveGender(
    ...
    if (gender !== 'male' && gender !== 'female' && gender !== 'userDefined') {
      throw new BadRequestException(
        'Gender must be of type string: male, female or userDefined',
      );
    }
  ...
  async saveTopics(userId: string, topics: Topics[]) {
    const existingUserFacts = await this.getUserFacts(userId);
